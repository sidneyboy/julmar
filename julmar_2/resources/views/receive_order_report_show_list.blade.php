<div class="card card-primary card-outline card-outline-tabs">
  <div class="card-header p-0 border-bottom-0">
    <ul class="nav nav-tabs" id="custom-tabs-three-tab" role="tablist">
      <li class="nav-item">
        <a class="nav-link active" id="custom-tabs-three-home-tab" data-toggle="pill" href="#custom-tabs-three-home" role="tab" aria-controls="custom-tabs-three-home" aria-selected="true">BO ALLOWANCE</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" id="custom-tabs-three-profile-tab" data-toggle="pill" href="#custom-tabs-three-profile" role="tab" aria-controls="custom-tabs-three-profile" aria-selected="false">INPUT VAT</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" id="custom-tabs-three-messages-tab" data-toggle="pill" href="#custom-tabs-three-messages" role="tab" aria-controls="custom-tabs-three-messages" aria-selected="false">RECEIVED SKU's</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" id="custom-tabs-three-settings-tab" data-toggle="pill" href="#custom-tabs-three-settings" role="tab" aria-controls="custom-tabs-three-settings" aria-selected="false">JOURNAL ENTRY REGISTER</a>
      </li>
    </ul>
  </div>
  <div class="card-body">
    <div class="tab-content" id="custom-tabs-three-tabContent">
      <div class="tab-pane fade show active" id="custom-tabs-three-home" role="tabpanel" aria-labelledby="custom-tabs-three-home-tab">
          ...
          <tbody>
                      ...
                      <div class="modal-dialog mw-100 w-75" role="document">
                        <div class="modal-content">
                          ...
                          <div class="modal-body">
                            <img src="{{  asset('/images/'. $data->invoice_image) }}" style="width:100%;">
                          </div>
              ...
              @foreach($bo_adjustment_data as $data)
                <tr>
                  <td style="text-align: center;">
                    <a href="{{ route('bo_allowance_adjustments_show_details', $data->id ."=". $data->principal->principal ."=". $data->particulars) }}" target="_blank">DM - BO {{ $data->id }}</a>
            ...
            @if($return_counter != 0)
              @foreach($return_order_data as $data)
                <tr>
                  <td style="text-align: center;"><a target="_blank" href="{{ route('return_to_principal_show_list_details', $data->id ."=". $data->principal->principal) }}">RET - {{ $data->id }}</a></td>
            ...
            @if($invoice_cost_counter != 0)
              @foreach($invoice_cost_data as $data)
                <tr>
                  <td style="text-align: center;">
                    {{-- <td style="text-align: center;"><a href="{{ route('received_order_report_show_details', $data->id ."=". $principal_name) }}" target="_blank">{{ "RR - ". $data->id }}</a></td> --}}
                    <a href="{{ route('invoice_cost_adjustments_show_details', $data->id ."=". $data->principal->principal ."=". $data->particulars) }}" target="_blank">INVOICE - ADJUSTMENT {{ $data->id }}</a>
                  ...
                  <td style="text-align: right;font-weight: bold;color">
                    ...
                    <span style="color:green;">{{ number_format($data->vatable_purchase,2,".",",") }}</span>
                    @else
                    <span style="color:red">{{ number_format($data->vatable_purchase,2,".",",") }}</span>
                    @endif
                  </td>
                  <td style="text-align: right;font-weight: bold;color">
                    @php
                    ...
                    <span style="color:green">{{ number_format($data->total_bo_allowance,2,".",",") }}</span>
                    @else
                    <span style="color:red">{{ number_format($data->total_bo_allowance,2,".",",") }}</span>
                    @endif
                  </td>
                </tr>
              @endforeach
        ...
        <a target="_blank" class="btn btn-success float-right" href="{{ route('received_order_report_print', $date_from ."=". $date_to ."=". $principal_id ."=". $principal_name ."=BO ALLOWANCE") }}">PRINT THIS</a>
      </div>
      ...
      <div class="tab-pane fade" id="custom-tabs-three-profile" role="tabpanel" aria-labelledby="custom-tabs-three-profile-tab">
          ...
          <tbody>
            @if ($received_counter != 0 )
              ...
              <tr>
                ...
                <td style="text-align: center;text-transform: uppercase;">
                  @if($data->invoice_image != '')
                  <button type="button" class="btn btn-link" data-toggle="modal" style="text-transform: uppercase;" data-target="#exampleModal_input_vat_id">
                  {{ $data->dr_si }}
                  </button>
                  <!-- Modal -->
                  <div class="modal fade" id="exampleModal_input_vat_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog mw-100 w-75" role="document">
                      <div class="modal-content">
                        <div class="modal-header">
                          <h5 class="modal-title" id="exampleModalLabel" style="color:blue;font-weight: bold;">INVOICE IMAGE</h5>
            ...
            @if($return_counter != 0)
              @for ($i = 0; $i < $return_counter; $i++)
                <tr>
                  <td style="text-align-last: center;"><a target="_blank" href="{{ route('return_to_principal_show_list_details', $return_order_data[$i]->id ."=". $return_order_data[$i]->principal->principal) }}">RET - {{ $return_order_data[$i]->id }}</a></td>
            ...
            @if($invoice_cost_counter != 0)
              ...
              <tr>
                <td style="text-align: center;">
                  <a href="{{ route('invoice_cost_adjustments_show_details', $invoice_cost_data[$i]->id ."=". $invoice_cost_data[$i]->principal->principal ."=". $invoice_cost_data[$i]->particulars) }}" target="_blank">INVOICE - ADJUSTMENT {{ $invoice_cost_data[$i]->id }}</a>
                </td>
                <td style="text-align: center;text-transform: uppercase;">
        ...
        <a target="_blank" class="btn btn-success float-right" href="{{ route('received_order_report_print', $date_from ."=". $date_to ."=". $principal_id ."=". $principal_name ."=VAT INPUTS") }}">PRINT THIS</a>
      </div>
            ...
            <tbody>
                @if($received_counter != 0)
                    @foreach($received_order_data as $received_data)
                      <tr>
                        <td style="text-align: center;">
                          <a target="_blank" href="{{ route('received_order_report_show_details', $received_data->id ."=". $principal_name) }}">RR - {{ $received_data->id }}</a>
                          ...
                          <div class="modal fade" id="exampleModal_received_deliveries_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog mw-100 w-75" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <img src="{{  asset('/images/'. $received_data->invoice_image) }}" style="width:100%;">
                ...
                @if($return_counter != 0)
                    @foreach($return_order_data as $return_data)
                       <tr>
                          <td style="text-align: center;">
                            <a target="_blank" href="{{ route('return_to_principal_show_list_details', $return_data->id ."=". $return_data->principal->principal) }}">RET - {{ $return_data->id }}</a>
                            
                          </td>
                          <td style="text-align: center;text-transform: uppercase;">
                            @if($return_data->received->invoice_image != '')
                            <button type="button" class="btn btn-link" data-toggle="modal" style="text-transform: uppercase;" data-target="#exampleModal_received_return_deliveries_id">
                            {{ $return_data->received->dr_si }}
                            </button>
                            <!-- Modal -->
                            <div class="modal fade" id="exampleModal_received_return_deliveries_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                              <div class="modal-dialog mw-100 w-75" role="document">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title" id="exampleModalLabel" style="color:blue;font-weight: bold;">INVOICE IMAGE</h5>
                                  ...
                                  <div class="modal-body">
                                    <img src="{{  asset('/images/'. $return_data->received->invoice_image) }}" style="width:100%;">
                                  </div>
                  ...
                  @foreach($bo_adjustment_data as $bo_data)
                    <tr>
                      ...
                      <td style="text-align: center;text-transform: uppercase;">
                        ...
                        <button type="button" class="btn btn-link" data-toggle="modal" style="text-transform: uppercase;" data-target="#exampleModal_received_bo_deliveries_id">
                        {{ $bo_data->received->dr_si }}
                        </button>
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal_received_bo_deliveries_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog mw-100 w-75" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel" style="color:blue;font-weight: bold;">INVOICE IMAGE</h5>
                              ...
                              <div class="modal-body">
                                <img src="{{  asset('/images/'. $bo_data->received->invoice_image) }}" style="width:100%;">
                              </div>
                 ...
                 @foreach($invoice_cost_data as $invoice_data)
                    <tr>
                      ...
                      <td style="text-align: center;text-transform: uppercase;">
                        ...
                        <button type="button" class="btn btn-link" data-toggle="modal" style="text-transform: uppercase;" data-target="#exampleModal_received_invoice_deliveries_id">
                        {{ $invoice_data->received->dr_si }}
                        </button>
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal_received_invoice_deliveries_id" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                          <div class="modal-dialog mw-100 w-75" role="document">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title" id="exampleModalLabel" style="color:blue;font-weight: bold;">INVOICE IMAGE</h5>
                              ...
                              <div class="modal-body">
                                <img src="{{  asset('/images/'. $invoice_data->received->invoice_image) }}" style="width:100%;">
                              </div>
          ...
          @if ($received_counter != 0)
            ...
            <tr>
              ...
              <td>
                @php
                echo $counter += 1;
                @endphp. INVENTORY <span style="text-transform: uppercase;">{{ $principal_name }}</span></td>
              ...
              <td></td>
              <td></td>
              <td>ACCOUNTS PAYABLE <span style="text-transform: uppercase;">{{ $principal_name }}</span></td>
              <td><span style="color:blue;font-style:italic">[ {{ $data->date }} ]</span></td>
              <td></td>
            ...
            @foreach ($return_order_data as $return)
              <tr>
                <td style="text-align: center"><a target="_blank" href="{{ route('return_to_principal_show_list_details', $return->id ."=". $return->principal->principal) }}">RET - {{ $return->id }}</a></td>
                <td >
                  @php
                  echo $counter += 1;
                  @endphp. ACCOUNTS PAYABLE  <span style="text-transform: uppercase;">{{ $principal_name }}</span></td>
                ...
                <td></td>
                <td></td>
                <td> INVENTORY <span style="text-transform: uppercase;">{{ $principal_name }}</span> </td>
                <td><span style="color:blue;font-style:italic">[ {{ $return->date }} ]</span></td>
                <td></td>
                ...
                <tr>
                  <td style="text-align: center"> <a href="{{ route('bo_allowance_adjustments_show_details', $bo->id ."=". $bo->principal->principal ."=". $bo->particulars) }}" target="_blank">DM - BO {{ $bo->id }}</a></td>
                  <td>
                    @php
                    echo $counter += 1;
                    @endphp. ACCOUNTS PAYABLE  <span style="text-transform: uppercase;">{{ $principal_name }}</span></td>
                  ...
                  <tr>
                    <td style="text-align: center"> <a href="{{ route('bo_allowance_adjustments_show_details', $invoice->id ."=". $invoice->principal->principal ."=". $invoice->particulars) }}" target="_blank">INVOICE - ADJUSTMENT {{ $invoice->id }}</a></td>
                    <td>
                      @php
                      echo $counter += 1;
                      @endphp. ACCOUNTS PAYABLE  <span style="text-transform: uppercase;">{{ $principal_name }}</span></td>
