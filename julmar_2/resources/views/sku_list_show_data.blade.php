@if($search_method == 'principal')
    <input type="hidden" id="second_parameter" value="{{ $second_parameter }}" >
    <input type="hidden" id="principal_name" value="{{ $principal_name }}">
    <table class="table table-bordered table-hover table-sm table-sm">
     ...
     {!! $sku_data->links() !!}
@elseif($search_method == 'sku_description' OR $search_method == 'sku_code')
    ...
    <table class="table table-bordered table-hover table-sm table-sm">
        ...
        <tbody>
            @for ($i=0; $i < $data_counter; $i++) 
                <tr>
                    <td style="text-align: center;">{{ $sku_data[$i]->id }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->sku_code }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->description }}</td>
                    <td style="text-align: center;">{{ $remaining_quantity[$i] }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->skuCategory->category }}</td>
                    ...
                    <td style="text-align: right;text-align: center;">
                        <button type="button" class="btn btn-link" data-toggle="modal" data-target="#exampleModal{{ $sku_data[$i]->id }}">
                        {{ number_format($sku_data[$i]->sku_price_details_one->unit_cost,4,".",",")  }}
                        </button>
                        <!-- Modal -->
                        <div class="modal fade" id="exampleModal{{ $sku_data[$i]->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                        ...
                                        <table class="table table-bordered table-hover table-sm">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="text-align: center;">PRICE 3</th>
                                                    <th style="text-align: center;">PRICE 4</th>
                                                    <th style="text-align: center;">PRICE 5</th>
                                                    <th style="text-align: center;">UPDATED</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach($sku_data[$i]->sku_price_details as $price_details)
                                                    <tr>
                                                        <td>{{ number_format($price_details->unit_cost,4,".",",") }}</td>
                                                        <td>{{ number_format($price_details->price_1,4,".",",") }}</td>
                                                        <td>{{ number_format($price_details->price_2,4,".",",") }}</td>
                                                        <td>{{ number_format($price_details->price_3,4,".",",") }}</td>
                                                        <td>{{ number_format($price_details->price_4,4,".",",") }}</td>
                                                        <td>{{ number_format($price_details->price_5,4,".",",") }}</td>
                                                        <td>
                                                            @if(is_null($price_details->created_at))
                                                                MIGRATED
                                                            @else
...
@elseif($search_method == 'category' OR $search_method == 'uom' OR $search_method == 'sku_type' OR $search_method == 'remarks')
    <input type="hidden" id="second_parameter" value="{{ $second_parameter }}" >
   {{--  <input type="hidden" id="principal_name" value="{{ $principal_name }}"> --}}
    <table class="table table-bordered table-hover table-sm">
        <thead>
        ...
        <tbody>
            @for ($i=0; $i < $data_counter; $i++) 
                <tr>
                    ...
                    <td style="text-align: center;">{{ $remaining_quantity[$i] }}</td>
                    <td style="text-transform: uppercase;text-align: center;">{{ $sku_data[$i]->skuCategory->category }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->skuPrincipal->principal }}</td>
                    <td style="text-transform: uppercase;text-align: center;">{{ $sku_data[$i]->unit_of_measurement }}</td>
                    <td style="text-transform: uppercase;text-align: center;">{{ $sku_data[$i]->sku_type }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->equivalent_sku_entryNo }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->equivalent_butal_pcs }}</td>
                    <td style="text-align: center;">{{ $sku_data[$i]->reorder_point }}</td>
                    <td style="text-align: right;text-align: center;">
                    ...
                    <td style="text-transform: uppercase;text-align: center;">{{ $sku_data[$i]->remarks }}</td>
                </tr>
            @endfor
        </tbody>
    </table>
