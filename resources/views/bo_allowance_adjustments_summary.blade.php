	...
	<tbody>
		@foreach ($sku as $data)
			<tr>
				...
				<td style="text-align: right;">{{ number_format($unit_cost_adjustment[$data],2,".",",") }} </td>
				<td style="text-align: right;">
					@php
				...
				<input type="hidden" name="bo_allowance_per_sku[{{ $data }}]" value="{{ $total_amount }}">
				</td>
			</tr>
		@endforeach
			<tr>
          ...
          <td style="font-weight: bold; text-align: right;font-size: 15px;border-bottom: 3px double #000000;">
            {{--  @php
             ...
             @endphp --}}
              ...
              @php
          	 	$vat_deduction =array_sum($sum_total_amount)*.12;
          	 @endphp
