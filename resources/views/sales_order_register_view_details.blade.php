<table class="table table-sm table-bordered table-hover" id="export_sales_register" style="text-align: center;">
	<thead>
		...
		<tr>
			...
			<th>{{  date('Y-m-d', strtotime($sales_invoice->delivered_date))  }}</th>
...
<div class="row">
	<div class="col-md-12">
		<button class="btn btn-success btn-block" style="font-weight: bold;" onclick="exportTableToCSV('{{  strtoupper($sales_invoice->agent->full_name) ." - ". strtoupper($sales_register_store_name) ." SALES REGISTER - ". $date   }}.csv')">EXPORT NOW</button>
	</div>
</div>
